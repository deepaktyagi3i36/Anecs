# = Entity description
#
# ReportMissingProductsJob find and report about not supported more products
# http://snippets.aktagon.com/snippets/305-Example-of-how-to-use-Ruby-s-NET-HTTP
#
...
#  TODO: exception
class ReportMissingProductsJob < AdminJobBase
  # ::Rails.logger.error("...")
  ...
  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("report_missing_products_job")
      if job
        if(attributes.nil?)
  ...
  private
  def process_products2
    yml_file = YAML::load(File.open("#{::Rails.root}/config/sportgora.yml"))
    url = URI.parse(yml_file[::Rails.env]["url"])
  ...
  def process_products1
    yml_file = YAML::load(File.open("#{::Rails.root}/config/sportgora.yml"))
    url = URI.parse(yml_file[::Rails.env]["url"])
    report_url_request(url)
    create_url_request(url)

    # Define page and per page constant
    page = 1
    per_page = 100
    ...
    while true
      ...
      request.set_form_data({:page => page, :per_page => per_page,
