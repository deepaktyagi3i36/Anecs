...
#
#
class UpdateManpagesJob < AdminJobBase
  # ::Rails.logger.error("...")

  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("update_manpages_job")
      if job
        if(attributes.nil?)
  ...
  private
  def start_recursion
    Manpage.find(:all, :conditions => ["parent_id IS NULL"]).each do |root|
      root.save!
  ...
  def continue_recursion(parent)
    Manpage.find(:all, :conditions => ["parent_id = ?", parent.id]).each do |child|
      child.save!
  ...
  def create_iteration(node)
    # Store mesage to database
    message = "Manpage are updated successfully, item id: #{node.id}, item name: #{node.name}"
