...
#
#
class UpdateArticlesJob < AdminJobBase
  # ::Rails.logger.error("...")

  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("update_articles_job")
      if job
        if(attributes.nil?)
  ...
  private
  def start_recursion
    Article.find(:all, :conditions => ["parent_id IS NULL"]).each do |root|
      root.save!
  ...
  def continue_recursion(parent)
    Article.find(:all, :conditions => ["parent_id = ?", parent.id]).each do |child|
      child.save!
  ...
  def create_iteration(node)
    message = "Article are updated successfully, item id: #{node.id}, item name: #{node.name}"
