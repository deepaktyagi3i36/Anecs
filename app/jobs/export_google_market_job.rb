...
class ExportGoogleMarketJob < AdminJobBase
  # ::Rails.logger.error("...")

  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("export_google_market_job")
      if job
        if(attributes.nil?)
  ...
  def export_market
    ...
    file = "#{::Rails.root.to_s}/tmp/markets/google.xml.gz"
    Zlib::GzipWriter.open(file) do |gz|
      gz.write "todo write data here"
    end

    # Move file from tmp to public (with no error)
    src = "#{::Rails.root.to_s}/tmp/markets/google.xml.gz"
    dest = "#{::Rails.root.to_s}/themes/#{@website.code}/public/google.xml.gz"
    FileUtils.mv src, dest, :force => true
  end

  def report_iteration
    ::Rails.logger.info("#{@admin_job.job} job is in progress. " +
      "Start google market export for #{@website.code}")
  end

  def create_iteration
    message = "#{@admin_job.job} job is in progress. Start google market export for #{@website.code}"
