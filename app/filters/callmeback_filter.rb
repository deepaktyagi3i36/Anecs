class CallmebackFilter < Tableless31Model
  # ::Rails.logger.error("...")
  ...
  def conditions
    ...
    sqlwhere = "1 = 1 "
    sqlwhere = sqlwhere + "and callmebacks.title like ? " if is_string_here?(title)
    sqlwhere = sqlwhere + "and callmebacks.created_at > ? " if is_object_here?(created_at_since)
    sqlwhere = sqlwhere + "and callmebacks.created_at < ? " if is_object_here?(created_at_till)
