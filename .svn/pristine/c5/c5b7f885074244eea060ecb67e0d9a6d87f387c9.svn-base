...
class ExportInsalesDataJob < AdminJobBase
  # ::Rails.logger.error("...")

  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("export_insales_data_job")
      if job
        if(attributes.nil?)
  ...
  def export_insales_data
    FileUtils.mkdir_p("#{::Rails.root.to_s}/tmp/databases/")
    file = "#{::Rails.root.to_s}/tmp/databases/insales_data.csv.gz"
    Zlib::GzipWriter.open(file) do |gz|
      gz.write "todo write data here"
    end

    # Move file from tmp to public (with no error)
    src = "#{::Rails.root.to_s}/tmp/databases/insales_data.csv.gz"
    dest = "#{::Rails.root.to_s}/themes/#{@website.code}/public/insales_data.csv.gz"
    FileUtils.mv src, dest, :force => true
  end

  def report_iteration
    ::Rails.logger.info("#{@admin_job.job} job is in progress. " +
      "Start insales data export for #{@website.code}")
  end

  def create_iteration
    message = "#{@admin_job.job} job is in progress. Start insales data export for #{@website.code}"
