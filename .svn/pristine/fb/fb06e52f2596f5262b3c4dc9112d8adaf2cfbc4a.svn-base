...
class SyncCabelasJob < AdminJobBase
  # ::Rails.logger.error("...")

  def initialize(attributes = nil)
    if(attributes.nil? || attributes[:admin_job_id].nil?)
      job = AdminJob.find_by_code("sync_cabelas_job")
      if job
        if(attributes.nil?)
  ...
  def process_products
    # TODO: here is algorithm begin: fetch products that belong to website
    # iterate accross all of them and fetch data from cabelas based on article
    return

    offset = 0
    yml_file = YAML::load(File.open("#{::Rails.root}/config/cabelas.yml"))
    url = URI.parse(yml_file[::Rails.env]["url"] + "/catalog/productList")
    report_url_request(url)
    create_url_request(url)

    # Get rows to fetch
    rows_to_fetch = @runtime_settings["sync_cabelas_rows_to_fetch"].to_i
  ...
  def check_price_in_delta(variant, price, currency)
    ...
    delta = @runtime_settings["cabelas_price_delta"]
